...
/**
 * @brief Run a single NumKong u8 L2 distance computation: C(m, n) = euclidean(A, B)
 *
 * B is pre-packed (norms stored in packed footer).
 * Each thread processes a contiguous chunk of rows from A.
 * Output is f32 euclidean distances (sqrt produces floats even from integer inputs).
 */
double RunNumKongL2(
    const uint8_t* a,
    const void* b_packed,
    ...
    const size_t c_stride = n * sizeof(float);
...
int main(int argc, char** argv) {
    ...
    // Same pack function as dots — norms are stored in the packed buffer footer
    ...
    for (int i = 0; i < WARMUP_RUNS; ++i) {
        RunNumKongL2(a.data(), b_packed.data(), m, n, d, out.data(), pool);
    }
    std::cout << " done" << std::endl;
    ...
    for (int i = 0; i < MEASURED_RUNS; ++i) {
        double ms = RunNumKongL2(a.data(), b_packed.data(), m, n, d, out.data(), pool);
        total_ms += ms;
