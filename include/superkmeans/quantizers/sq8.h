...
#pragma omp parallel for num_threads(g_n_threads)
    ...
    void FindNearestNeighbor(
        ...
        const size_t max_packed_size = nk_dots_packed_size_u8(Y_BATCH_SIZE, d);
        ...
        for (size_t i = 0; i < n_x; i += X_BATCH_SIZE) {
            ...
            for (size_t j = 0; j < n_y; j += Y_BATCH_SIZE) {
                ...
                nk_dots_pack_u8(y + j * d, batch_n_y, d, d, y_packed.data());
    ...
    void FindNearestNeighborWithReranking(
        ...
        for (size_t i = 0; i < n_x; i += X_BATCH_SIZE) {
            ...
            for (size_t j = 0; j < n_y; j += Y_BATCH_SIZE) {
                ...
                const size_t packed_size = nk_dots_packed_size_u8(batch_n_y, d);
                if (packed_size > y_packed.size()) y_packed.resize(packed_size);
                nk_dots_pack_u8(y_quantized + j * d, batch_n_y, d, d, y_packed.data());
